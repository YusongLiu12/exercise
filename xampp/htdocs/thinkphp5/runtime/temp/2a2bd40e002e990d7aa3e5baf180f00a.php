<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:74:"D:\xampp\htdocs\thinkphp5\public/../application/index\view\klass\edit.html";i:1689669194;}*/ ?>
<!DOCTYPE html>
<html lang="zh-cn">
<head>
    <meta charset="UTF-8">
...
<body>
    <form action="<?php echo url('update'); ?>" method="post">
        ...
        <label for="name">name:</label><input type="text" name="name" id="name" value="<?php echo $Klass->getData('name'); ?>" />
        ...
        <select name="teacher_id" id="teacher">
           <?php if(is_array($teachers) || $teachers instanceof \think\Collection): $i = 0; $__LIST__ = $teachers;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$_Teacher): $mod = ($i % 2 );++$i;?>
            <option value="<?php echo $_Teacher->getData('id'); ?>" 
                <?php if($_Teacher->getData('id') == $Klass->getData('teacher_id')): ?> selected="selected" <?php endif; ?>
                >
